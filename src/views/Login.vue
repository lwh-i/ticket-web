      ...
      >
        ...
        <el-form-item style="width: 100%">
          ...
          <el-button
              :loading="loading"
              size="medium"
              style="width: 100%;background-color: #5839ac;color: #ffffff;margin-left: 0px"
...
import{ticketLogin} from "../api/goods/goodsApi";
//import md5 from "../views/"
export default {
  name: "Login",
  data() {
    return {
...
,
      ...
      this.loading = true;
       const salt="1234";
			  const str=""+salt.charAt(0)+salt.charAt(2)+this.loginVo.password;
			  const password=md5(str);
        console.log("passmd5="+password)
      let captcha = new TencentCaptcha(this.config.TENCENT_CAPTCHA, function (res) {
        if (res.ret === 0) {
              that.loginVo.password=password
               console.log("点击了"+that.loginVo.phone+that.loginVo.password)
              ticketLogin(that.loginVo).then((res) => {
